...
The model from eqn. 2.2 can be easily fitted using the \texttt{lm()} function:
...
GLMs can be fitted using the \texttt{glm()} function and here we fit the model from eqn. 2.3:
\begin{knitrout}
\color{fgcolor}\small\begin{kframe}
\begin{alltt}
...
\subsubsection{Model fitting}
Fitting a quasi-Poisson model (eqn. 2.4) is straight forward:
\begin{knitrout}
\color{fgcolor}\small\begin{kframe}
\begin{alltt}
...
\subsubsection{Model fitting}
To fit a negative binomial GLM (eqn. 2.5) we could use \texttt{glm.nb()} from the MASS package \citep{venables_modern_2002}:
\begin{knitrout}
\color{fgcolor}\small\begin{kframe}
\begin{alltt}
...
First, we arcsine transform (eqn. 2.6) the proportions:
\begin{knitrout}
\color{fgcolor}\small\begin{kframe}
\begin{alltt}
...
The binomial model with a logit link (eqn. 2.7) between predictors and response can be fitted using the \texttt{glm()} function:
\begin{knitrout}
\color{fgcolor}\small\begin{kframe}
\begin{alltt}
...
Here the weights arguments, indicates how many fish where exposed in each treatment (N=10, eqn 2.7).
