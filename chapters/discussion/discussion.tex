...
Clearly, further advances in the statistical evalation of mesocosm experiments is needed.
...
In chapter~\ref{sec:smallstreams} we compiled and analysed monitoring data  leading to the currently most extensive dataset available for Germany. 
We demonstrated, that in the current monitoring scheme small streams below $10~km^2$are underrepresented (Figure~\ref{fig:ss:fig3}, top).
...
% pesticide dynamiken
Our results revealed that the signal from agricultural pesticides can be detected down to a small percentage of agriculture within the catchment (Figure~\ref{fig:ss:fig4}).
...
We were to detect a small, but distinct increase of risks after precipitation events. 
...
Our results suggest that absolute measured concentrations are subject to a high error due to the sampling process and adds evidence that current monitoring schemes, largely unconnected from precipitation events underestimate pesticide risks \citep{xing_influences_2013, stehle_probabilistic_2013}. 
...
However, current monitoring mainly addresses streams bigger than considered in ERA.
Our results indicated that small streams are frequently exposed to high risks from pesticides.
However, to provide a suitable feedback for ERA small agricultural streams must be integrated into environmental monitoring schemes. 
Moreover, because of the outlined shortcomings of chemical monitoring data, absolute values are not comparable. 
As the measurements within the current monitoring schemes provide underestimations all exceedances of risk thresholds represent an unacceptable risk. 

Risk thresholds in chapter~\ref{sec:smallstreams} were particularly exceeded for the organophosphate Chlorpyrifos and neonicotinoid substances. 
...
Recent studies investigating large-scale pesticide risks did not consider neonicotinoid insecticides \citep{malaj_organic_2014, stehle_pesticide_2015} and likely missed these risks. 
However, this shows also that the analysed spectrum is an important driver of detected compounds \citep{schreiner_pesticide_2016, malaj_organic_2014}, that must be taken into account when evaluating monitoring data for risk assessment. 
The WFD currently considers on a few (45), relatively well-known substances \citep{european_union_directive_2013} and a status assessment based on EQS likely missed the actual chemical pollution \citep{moschet_how_2014}. 
...
Monitoring data provide also valuable information for the priorisation of emerging pollutants and future monitoring with the WFD \citep{brack_towards_2017}.

% Biological Monitoring
Additionally, monitoring under the WFD is also performed for biological components of freshwaters and a combination with pesticide exposure data might provide valuable insights into large-scale field effects of chemical substances \citep{schipper_deriving_2014}.
...
Integration monitoring data and risk assessment data is a mandatory requirement for landscape level ecotoxicology and risk assessment \citep{cairns_developing_1996, focks_challenge:_2014} and needed for an improved model development and validation \citep{knabel_regulatory_2012, brock_aquatic_2006}. 
...
The webchem package, presented in chapter~\ref{sec:webchem}, can foster such an integration, however data must also be accessible. 
...
However many other data sources are available and implementation of more than 15 other data sources is in the pipeline will be implemented if time permits.
